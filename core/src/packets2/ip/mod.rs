...
use crate::packets2::checksum::PseudoHeader;
use anyhow::Result;
use std::fmt;
...
mod tests {
    ...
    fn reverse_flow() {
        ...
        flow.set_src_ip(Ipv4Addr::new(127, 0, 0, 1).into());
        flow.set_dst_ip(Ipv4Addr::new(8, 8, 8, 8).into());
        flow.set_src_port(12345);
        flow.set_dst_port(53);
        flow.set_protocol(ProtocolNumbers::Udp);
        let reverse = flow.reverse();

        assert_eq!(flow.src_ip(), reverse.dst_ip());
        assert_eq!(flow.dst_ip(), reverse.src_ip());
        assert_eq!(flow.src_port(), reverse.dst_port());
        assert_eq!(flow.dst_port(), reverse.src_port());
        assert_eq!(flow.protocol(), reverse.protocol());
    }
}
