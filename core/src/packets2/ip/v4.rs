...
use crate::packets2::checksum::{self, PseudoHeader};
use crate::packets2::ethernet::{EtherTypes, Ethernet};
use crate::packets2::ip::{IpPacket, ProtocolNumber, DEFAULT_IP_TTL};
use crate::packets2::{Datalink, Packet, SizeOf};
use anyhow::Result;
use std::fmt;
...
pub struct Ipv4<'env, E: Packet<'env> + Datalink = Ethernet<'env>> {
    envelope: &'env mut E,
    header: NonNull<Ipv4Header>,
    offset: usize,
}

impl<'env, E: Packet<'env> + Datalink> Ipv4<'env, E> {
    #[inline]
...
impl<'env, E: Packet<'env> + Datalink> fmt::Debug for Ipv4<'env, E> {
...
impl<'env, E: Packet<'env> + Datalink> Packet<'env> for Ipv4<'env, E> {
    type Envelope = E;
    ...
    /// # Errors
    ///
    /// Returns `Ipv4Error::InvalidPacket` if the envelope's next header is
    /// not set to [`EtherTypes::Ipv4`].
    /// Returns `BufferError::OutOfBuffer` if the IPv4 header is larger than
    /// the data payload.
    ///
    ...
    fn try_parse(envelope: &'env mut Self::Envelope) -> Result<Self> {
            ...
            envelope.next_header() == EtherTypes::Ipv4,
            Ipv4Error::InvalidPacket
        );
    ...
    /// enough free space for the IPv4 header.
    ///
    /// [`ether_type`]: Ethernet::ether_type
    /// [`EtherTypes::Ipv4`]: EtherTypes::Ipv4
    #[inline]
    fn try_push(envelope: &'env mut Self::Envelope) -> Result<Self> {
        ...
        envelope.set_next_header(EtherTypes::Ipv4);
...
impl<'env, E: Packet<'env> + Datalink> IpPacket for Ipv4<'env, E> {
    #[inline]
    ...
    #[inline]
    fn set_next_header(&mut self, proto: ProtocolNumber) {
    ...
    /// # Errors
    ///
    /// Returns `Ipv4Error::InvalidIpAddr` if `src` is not an Ipv4Addr.
    #[inline]
    fn set_src(&mut self, src: IpAddr) -> Result<()> {
        match src {
            ...
            _ => Err(Ipv4Error::InvalidIpAddr.into()),
        }
    }
    ...
    /// # Errors
    ///
    /// Returns `Ipv4Error::InvalidIpAddr` if `dst` is not an Ipv4Addr.
    #[inline]
    ...
    /// # Errors
    ///
    /// Returns `Ipv4Error::MtuTooSmall` if the desired MTU is less than
    /// [`IPV4_MIN_MTU`].
    ///
    ...
    fn truncate(&mut self, mtu: usize) -> Result<()> {
            ...
            mtu >= IPV4_MIN_MTU,
            Ipv4Error::MtuTooSmall(mtu, IPV4_MIN_MTU)
        );
...
impl<'env, E: Packet<'env> + Datalink> Ipv4Packet for Ipv4<'env, E> {}
...
#[derive(Debug, Error)]
pub enum Ipv4Error {
    /// The envelope's payload is not an IPv4 packet.
    #[error("The payload is not an IPv4 packet.")]
    InvalidPacket,
    ...
    /// MTU is less than the minimum IPv4 MTU.
    #[error("MTU {0} cannot be less than the minimum IPv4 MTU {1}.")]
...
mod tests {
    use super::*;
    use crate::packets2::ip::ProtocolNumbers;
    ...
    fn parse_ipv4_packet() {
        ...
        let ip4 = ethernet.parse::<Ipv4<'_>>().unwrap();
    ...
    fn parse_non_ipv4_packet() {
        let mut packet = Mbuf::from_bytes(&TCP6_PACKET).unwrap();
        let mut ethernet = packet.parse::<Ethernet<'_>>().unwrap();

        assert!(ethernet.parse::<Ipv4<'_>>().is_err());
    }
    ...
    fn push_ipv4_packet() {
        ...
        let ip4 = ethernet.push::<Ipv4<'_>>().unwrap();
