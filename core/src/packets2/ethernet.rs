...
use crate::packets2::{BufferError, Datalink, Mbuf, Packet, SizeOf};
use anyhow::Result;
use std::fmt;
use std::ptr::NonNull;
...
pub struct Ethernet<'env> {
    envelope: &'env mut Mbuf,
...
impl<'env> Packet<'env> for Ethernet<'env> {
    ...
    /// # Errors
    ///
    /// Returns `BufferError::OutOfBuffer` if the Ethernet header is larger
    /// than the data payload.
    #[inline]
    ...
    /// enough free space for the Ethernet header.
    #[inline]
...
mod tests {
    ...
    fn parse_dot1q_packet() {
        let mut packet = Mbuf::from_bytes(&VLAN_DOT1Q_PACKET).unwrap();
        let ethernet = packet.parse::<Ethernet<'_>>().unwrap();
    ...
    fn parse_qinq_packet() {
        let mut packet = Mbuf::from_bytes(&VLAN_QINQ_PACKET).unwrap();
        let ethernet = packet.parse::<Ethernet<'_>>().unwrap();
    ...
    fn push_ethernet_packet() {
        ...
        let ethernet = packet.push::<Ethernet<'_>>().unwrap();
    ...
    fn bug_push_ethernet_corrupts_buffer() {
        let data = [255u8; 8];
        let mut packet = Mbuf::from_bytes(&data).unwrap();
        let ethernet = packet.push::<Ethernet<'_>>().unwrap();
